...
interface GetOrGenerateRecipeResponse {
  success: boolean;
  recipe: Recipe;
...
function RecipeContent() {
  ...
  const {
    ...
    data: recipeData,
    fn: fetchRecipe,
  } = useFetch<GetOrGenerateRecipeResponse, [FormData]>(getOrGenerateRecipe);
  ...
  const {
    ...
    data: saveData,
    fn: saveToCollection,
  } = useFetch<SaveRecipeResponse, [FormData]>(saveRecipeToCollection);
  ...
  } = useFetch<RemoveRecipeResponse, [FormData]>(removeRecipeFromCollection);
  ...
  useEffect(() => {
    if (recipeData?.success) {
      ...
      setRecipeId(String(recipeData.recipeId));
  ...
  const isProUser = Boolean(recipeData?.isPro);
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white p-6 border-2 border-stone-200 lg:sticky lg:top-24">
              ...
              {/* Group by category */}
              {Object.entries(
                recipe.ingredients.reduce<Record<string, RecipeIngredient[]>>(
                  (acc, ing) => {
              ...
              ).map(([category, items]: [string, RecipeIngredient[]]) => (
