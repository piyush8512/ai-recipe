...
async function fetchRecipeImage(recipeName: string) {
  try {
...
export async function getOrGenerateRecipe(formData: FormData) {
  try {
    ...
    const recipeNameValue = formData.get("recipeName");
    if (typeof recipeNameValue !== "string" || !recipeNameValue.trim()) {
      throw new Error("Recipe name is required");
    }

    // Normalize the title (e.g., "apple cake" → "Apple Cake")
    const normalizedTitle = normalizeTitle(recipeNameValue);
...
export async function saveRecipeToCollection(formData: FormData) {
  try {
    ...
    if (typeof recipeIdValue !== "string" || !recipeIdValue.trim()) {
      throw new Error("Recipe ID is required");
    }
    const recipeId = recipeIdValue;
...
export async function getSavedRecipes() {
  try {
    ...
    const recipes = data.data
      .map((savedRecipe: { recipe: unknown }) => savedRecipe.recipe)
