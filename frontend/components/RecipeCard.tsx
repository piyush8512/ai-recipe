...
interface RecipeCardProps {
  recipe: any; // You can replace 'any' with a more specific type based on your data structure
  variant?: "grid" | "pantry" | "list" | "default";
}
...
export default function RecipeCard({
  ...
  const getRecipeData = (): RecipeData => {
    ...
    if (recipe.strMeal) {
      return {
        ...defaultData,
        title: recipe.strMeal || defaultData.title,
    ...
    if (recipe.matchPercentage) {
      return {
        ...defaultData,
        title: recipe.title || defaultData.title,
            ...
            <div className="flex gap-4 text-sm text-stone-500">
              {(data.prepTime || data.cookTime) && (
                <div className="flex items-center gap-1">
                  ...
                  <span>
                    {Number(data.prepTime) + Number(data.cookTime)} mins
                  </span>
                </div>
              )}
              {data.servings && (
